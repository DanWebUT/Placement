...
def calculate_path(robot_starting_positions, robot_goal_positions,moving_robots,printing_chunks,robot_schedules,robot_positions,robot_last_chunk_printed):
    robot_path_lengths = np.zeros(len(robot_schedules),int)
    for count, robot in enumerate(moving_robots):
        starting_x = robot_starting_positions[count][0]
        starting_y = robot_starting_positions[count][1]
        ending_x = robot_goal_positions[count][0]
        ending_y = robot_goal_positions[count][1]
        robot_path_lengths[robot] = abs(starting_x-ending_x)+abs(starting_y-ending_y)
        robot_schedules[robot].append(printing_chunks[count])
        robot_last_chunk_printed[robot] = printing_chunks[count]
...
def schedule(robot_starting_positions, floor_size, chunk_dependencies, chunk_job, chunk_print_time, chunk_positions, print_direction):
    ...
    robot_last_chunk_printed = np.zeros(len(robot_starting_positions),int)
    robot_positions = np.zeros([len(robot_starting_positions),2],int)
    ...
    try:
        ...
        while min(chunk_print_states) <= 1 and iteration_count <=50:
            ...
            if min(robot_states) == 0:
                ...
                if printable_chunks != []:
                    ...
                    (robot_path_lengths,robot_schedules,robot_positions,robot_last_chunk_printed) = calculate_path(robot_starting_positions, robot_goal_positions,moving_robots,printing_chunks,robot_schedules,robot_positions,robot_last_chunk_printed)
        ...
        total_print_time = 20000
    except IndexError:
        total_print_time = 30000
